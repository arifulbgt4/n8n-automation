...
async function provisionTenantMediaUser(tenantId: string, name: string) {
  ...
  const response = await fetch(`${mediaBase()}/api/v1/admin/users`, {
    ...
    body: JSON.stringify({ name: `tenant-${tenantId}-${name}`.slice(0, 80), quota_bytes: null }),
  });
...
export async function mediaRoutes(app: FastifyInstance) {
  ...
  app.get("/v1/tenants/:tenantId/media", async (request, reply) => {
    ...
    `, [tenantId, q.businessId ?? null, q.limit, q.offset]);
  ...
  app.get("/v1/tenants/:tenantId/media/:assetId/content", async (request, reply) => {
    ...
    const asset = await query<{ storage_file_id: string; mime_type: string; original_name: string | null }>("SELECT storage_file_id,mime_type,original_name FROM media_assets WHERE id=$1 AND tenant_id=$2 AND processing_status<>'deleted'", [params.assetId, params.tenantId]);
