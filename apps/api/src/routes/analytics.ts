    ...
    const [usage, conversations, outcomes, channels] = await Promise.all([
      query(`
        ...
        WHERE tenant_id=$1 AND occurred_at >= $2 AND occurred_at <= $3
          AND ($4::uuid IS NULL OR business_id=$4) AND ($5::uuid IS NULL OR channel_account_id=$5) AND ($6::uuid[] IS NULL OR business_id=ANY($6::uuid[]))
      ...
      query(`
        SELECT
          (SELECT count(*)::int FROM orders WHERE tenant_id=$1 AND created_at BETWEEN $2 AND $3 AND ($4::uuid IS NULL OR business_id=$4) AND ($5::uuid IS NULL OR channel_account_id=$5) AND ($6::uuid[] IS NULL OR business_id=ANY($6::uuid[]))) AS orders,
          (SELECT count(*)::int FROM bookings WHERE tenant_id=$1 AND created_at BETWEEN $2 AND $3 AND ($4::uuid IS NULL OR business_id=$4) AND ($5::uuid IS NULL OR channel_account_id=$5) AND ($6::uuid[] IS NULL OR business_id=ANY($6::uuid[]))) AS bookings,
          (SELECT count(*)::int FROM leads WHERE tenant_id=$1 AND created_at BETWEEN $2 AND $3 AND ($4::uuid IS NULL OR business_id=$4) AND ($5::uuid IS NULL OR channel_account_id=$5) AND ($6::uuid[] IS NULL OR business_id=ANY($6::uuid[]))) AS leads
      `, [tenantId, q.from, q.to, q.businessId ?? null, q.channelId ?? null, scope]),
      query(`
  ...
  app.get("/v1/tenants/:tenantId/analytics/timeseries", async (request, reply) => {
    ...
    const result = await query(`
      ...
      FROM usage_events WHERE tenant_id=$1 AND occurred_at BETWEEN $2 AND $3
        AND ($4::text IS NULL OR event_type=$4) AND ($5::uuid IS NULL OR business_id=$5) AND ($6::uuid IS NULL OR channel_account_id=$6) AND ($7::uuid[] IS NULL OR business_id=ANY($7::uuid[]))
      GROUP BY day,event_type ORDER BY day,event_type
    `, [tenantId, q.from, q.to, q.eventType ?? null, q.businessId ?? null, q.channelId ?? null, scope]);
